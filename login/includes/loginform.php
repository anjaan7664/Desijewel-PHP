        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] >= '1') {
                ...
                //Success! Register $myusername, $mypassword and return "true"
                    $success = 'true';
                    setcookie("username",$myusername,strtotime('+200 days'), "/", "", "", TRUE);
                    setcookie("password",$result['password'],strtotime('+200 days'), "/", "", "", TRUE);

            } elseif (password_verify($mypassword, $result['password']) && $result['verified'] == '0') {

                //Account not yet verified
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>You cannot log in until your account has been verified . Please Contact/Whatsapp on <ul><li>7793055835</li><li>9521522736</li><li>9571790992</li></ul> to verify your account .</div>";
